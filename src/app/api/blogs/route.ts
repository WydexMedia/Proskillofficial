...
export async function POST(req: NextRequest) {
  const { title, description, content, image, imageAlt } = await req.json();
...
export async function PATCH(req: NextRequest) {
  const { id, title, description, content, image, imageAlt } = await req.json();
  ...
  const result = await db.collection('blogs').updateOne(
    { _id: new ObjectId(id) },
    { $set: { title, description, content, image: image || '', imageAlt: imageAlt || '', updatedAt: new Date() } }
  );
