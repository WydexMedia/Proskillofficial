...
// Simple image preview component for admin
function AdminImagePreview({ imageKey }: { imageKey: string }) {
  ...
  if (loading) {
    return <div className="mx-auto h-24 w-32 bg-gray-200 animate-pulse rounded"></div>;
  }
...
export default function AdminPage() {
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      ...
      const uploadResponse = await fetch('/api/s3/blog-upload', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          filename: file.name,
  ...
  const handleAddBlog = async (e: React.FormEvent) => {
    ...
    const res = await fetch('/api/blogs', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title: blogTitle, description: blogDesc, image: blogImage, imageAlt: blogImageAlt, content: blogContent })
    });
    const data = await res.json();
  ...
  const handleEditBlogSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch('/api/blogs', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: editBlog._id, title: blogTitle, description: blogDesc, image: blogImage, imageAlt: blogImageAlt, content: blogContent })
    });
    const data = await res.json();
                      ...
                      >
                        {uploadingImage ? (
                          ...
                          <div className="space-y-2">
                            ...
                            <AdminImagePreview imageKey={blogImage} />
                          </div>
                        ) : (
                          <div>
